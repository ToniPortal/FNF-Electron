...
function createWindow() {
  ...
  ipcMain.on("enregistre", (event, data) => {
    const current = require('./data/listgame.json');
    ...
    try {
      ...
      if (verif == 0) {
        ...
        fs.writeFileSync('./data/listgame.json', JSON.stringify(current, null, 4));
  ...
  ipcMain.on("deleteall", (event, data) => {
    try {
      fs.writeFile('./data/listgame.json', JSON.stringify({ allgame: [] }, null, 4), function (err) {
        if (err) throw err;
        console.log('Saved!');
      });
    } catch (err) {
  ...
  // var pathscore = "%APPDATA%/Roaming/ShadowMario/FNF' OneShot/ninjamuffin99/funkin.sol"
